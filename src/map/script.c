...
BUILDIN(monster)
{
	...
	if (sd && strcmp(mapn, "this") == 0)
		...
		if ((m = script->mapname2mapid(st, mapn)) == -1)
			return false;
...
BUILDIN(playbgmall) {
	...
	if( script_hasdata(st,7) ) {
		...
		if ((m = script->mapname2mapid(st, mapname)) == -1)
			return false;
...
#define setunitdata_check_string(arg) \
	...
	case UDT_MASTERCID:
		...
		break;
	case UDT_MAPIDXY:
		if ((val = script->mapname2mapid(st, mapname)) == -1)
			return false;
...
BUILDIN(instance_set_respawn)
{
	...
	if( instance_id == -1 || !instance->valid(instance_id) )
		script_pushint(st, 0);
	else if ((mid = script->mapname2mapid(st, map_name)) == -1) {
		script_pushint(st, 0);
		return false;
	} else {
		int i;
...
/**
 * the above function return mapindex, this mapname2mapid function return mapid
 **/
int16 script_mapname2mapid(struct script_state *st, const char *name)
{
